...
int main (int argc, char *argv[]) {
    ...
    while (1) {
        bool skip;              // Set to true if we should skip processing
                                // the rest of the command line, for various reasons.
        ...
        if (!skip) {
            ...
            if (is_built_in(cmd_parts->cmd)) {
                ...
                // get_fg_only() returns the file-scoped variable in fg_only_mode.c
                // that indicates whether or not we are in fg only mode.
